...
- **Rate limiting** реализован на уровне приложения поверх Redis, а не на инфраструктурном шлюзе — уязвим к обходу при горизонтальном масштабировании, но проще в управлении и тестировании. Реальный IP клиента берётся из `X-Forwarded-For` **только если** прямой пир — доверенный прокси (localhost/приватная сеть, т.е. Caddy/nginx); в противном случае заголовок игнорируется, чтобы клиент не мог его подделать и обойти лимит.
...
**Прод (docker-compose.yml):** наружу торчит только Caddy (`80`/`443`) — он сам выпускает TLS-сертификат для `CADDY_DOMAIN` и проксирует на `app:8080`, передавая реальный IP клиента в `X-Forwarded-For`. Порт `8080` приложения наружу не публикуется. Rate limiter бэкенда доверяет `X-Forwarded-For` только от доверенного прокси (loopback/приватная сеть), поэтому Caddy в той же сети корректно обрабатывается.
...
COOKIE_SECURE=false
# Домен для Caddy (прод, docker-compose.yml): Caddy выпустит TLS-сертификат сам
CADDY_DOMAIN=tmatch.space
...
# CORS
# В проде — https://домен из CADDY_DOMAIN
